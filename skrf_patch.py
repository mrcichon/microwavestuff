# scikit-rf >0.17.0 changed time_gate/delay; restore the v0.17 behaviour the tabs rely on.
# import for side effect: binds rf.Network.time_gate/.delay, so any importer gets the patch.
import numpy as np
import skrf as rf
...
if rf.__version__ > "0.17.0":
    ...
    def time_gate_v017(self, center=None, span=None, **kwargs):
        ...
        out = self.copy()
        if center != 0:
            out = delay_v017(out, -center, 'ns', port=0, media=kwargs.get('media'))
